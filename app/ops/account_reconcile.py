...
def _direction(side):
    """+1 for a buy, -1 for a sell, and a refusal to guess when the side is missing.

    This used to be a two-branch expression whose else threw the fill on the SELL side:
    every record with no usable side was silently negated. The stored fills were exactly
    that shape -- 188 of them carried `"side": null` -- so the ledger this function is
    supposed to total came back with every quantity inverted, and the reconciliation built
    on it reported a mismatch on every position it checked.

    Returning None instead moves the fault to the caller, which can report "the side is
    missing" as the finding. A calibration check that quietly changes its own input is
    worse than one that has nothing to say.
    """
...
def signed_fills(fills):
    ...
    Fills whose side cannot be established are skipped rather than assumed. They used to be
    counted as sells, which turned a missing field into a wrong number.
    """
    totals = {}
...
def order_position_mismatches(account, orders, tolerance=QUANTITY_TOLERANCE):
    ...
    for order in orders or ():
        ...
        if filled <= 0:
            continue
        direction = _direction(getattr(order, 'side', ''))
        if direction is None:
            # An order with no side cannot be placed on either side of the ledger. It was
            # counted as a sell, which is how a missing field became a position mismatch.
            unknown_side.append(symbol)
            continue
        fills_by_symbol[symbol] = fills_by_symbol.get(symbol, 0.0) + direction * filled
    mismatches = []
    for symbol in sorted(set(fills_by_symbol) | set(unknown_side)):
        want = fills_by_symbol.get(symbol, 0.0)
        have = held.get(symbol, 0.0)
        if symbol in unknown_side:
            mismatches.append({'symbol': symbol, 'from_orders': None, 'held': have,
                               'difference': None, 'reason': 'order_side_missing'})
            continue
